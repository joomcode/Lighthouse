...
- (void)setupRouter {
    ...
    [driverProvider bindNode:hierarchy.rootNode toBlock:^id<LHDriver>(id<LHNode> node) {
        return [[LHModalPresentationDriver alloc] initWithWindow:self.window];
    }];
    
    [driverProvider bindNodeClass:[LHStackNode class] toBlock:^id<LHDriver>(id<LHNode> node) {
        return [[LHNavigationControllerDriver alloc] init];
    }];
    
    [driverProvider bindNodeClass:[LHTabNode class] toBlock:^id<LHDriver>(id<LHNode> node) {
        return [[LHTabBarControllerDriver alloc] init];
    }];
    
    [driverProvider bindNodes:@[ hierarchy.redNode, hierarchy.anotherRedNode ] toBlock:^id<LHDriver>(id<LHNode> node) {
        return [[PXStateViewControllerDriver alloc] initWithViewControllerClass:[PXRedViewController class]];
    }];
    
    [driverProvider bindNodes:@[ hierarchy.greenNode, hierarchy.anotherGreenNode ] toBlock:^id<LHDriver>(id<LHNode> node) {
        return [[PXStateViewControllerDriver alloc] initWithViewControllerClass:[PXGreenViewController class]];
    }];
    
    [driverProvider bindNodes:@[ hierarchy.blueNode, hierarchy.anotherBlueNode ] toBlock:^id<LHDriver>(id<LHNode> node) {
        return [[PXStateViewControllerDriver alloc] initWithViewControllerClass:[PXBlueViewController class]];
    }];
    
    [driverProvider bindNodes:@[ hierarchy.modalNode ] toBlock:^id<LHDriver>(id<LHNode> node) {
        return [[PXStateViewControllerDriver alloc] initWithViewControllerClass:[PXModalViewController class]];
    }];
    
    [driverProvider bindNode:hierarchy.deepModalNode toBlock:^id<LHDriver>(id<LHNode> node) {
